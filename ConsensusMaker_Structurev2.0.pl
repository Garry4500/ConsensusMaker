...
$con=<STDIN>;
print "Sollen Luecken zum Consensus gezaehlt werden? (yes/no)\n";
$lucke=<STDIN>;
...
if ($con < 51)
{
    print "\nDieser Consensus ist nicht moeglich.\n\n"; 
}
else
{
#Neue Dateien aus .xfasta Alignment erstellen
    ...
    print "Das Alignment umfasst $zahl Sequenzen.\n\nConsensus wird berechnet.\n";
...
#Lücken bei Bedarf entfernen (nicht fertig)

    if ($lucke eq "no")
    {
	print "\n";
	foreach my $ll (0 .. "$#matrix")
	{
	    foreach my $zl (0 .. "$#{$matrix[0]}")
	    {
		if ("${$matrix[$ll]}[$zl]" eq "-")
		{
		    print "Lücke erkannt!\n";
	...
	my %seen;
	@lucken = grep { ! $seen{$_}++ } @lucken;

	foreach my $l (0 .. "$#matrix")
	{
	    my @zeilelucke;
	    @zeilelucke = ();
	    foreach my $z (0 .. "$#{$matrix[0]}")
	    {
		my $li = 0;

		for my $luckenbay (@lucken)
		{
		    if ("$luckenbay" eq "$z")
		    {
			print "Lücke entfernt!\n";
			$li = $li + 1;
		    }
		}
		if ($li == 0)
		{
		    push (@zeilelucke, "${$matrix[$l]}[$z]");
		}
	    }
	    
	    my $ref_zeilelucke = \@zeilelucke;
	    push (@matrixlucketemp, $ref_zeilelucke);

	}
	
	@matrix = ();
	@matrix = @matrixlucketemp;
